od_A_VDM <- function(Fx, w1, eff, it.max, t.max, track) {
    ...
    if (track) {
        info <- paste("Running od_A_VDM for cca", t.max, "seconds")
    ...
    w <- w1; M <- crossprod(sqrt(w) * Fx); M.inv <- solve(M)
    a.fun <- as.vector(((Fx %*% M.inv)^2) %*% one)

    while (TRUE) {
        n.iter <- n.iter + 1; w <- n.iter * w/(n.iter + 1)
        i.max <- which.max(a.fun); w[i.max] <- w[i.max] + 1/(n.iter + 1)
        M <- crossprod(sqrt(w) * Fx); M.inv <- solve(M)
        a.fun <- ((Fx %*% M.inv)^2) %*% one
        eff.act <- sum(diag(M.inv))/max(a.fun)
        tm <- as.numeric(proc.time()[3]) - start    
        if (track && tm > next.sec) {
            print(paste("od_A_VDM Time:", round(tm, 1), 
    ...
    Phi.best <- optcrit(Fx, w, crit = "A", echo = FALSE)
    
    if (track) {
        info <- paste("od_A_VDM finished after", 
