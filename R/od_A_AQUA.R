od_A_AQUA <- function(Fx, b1, A1, b2, A2, b3, A3, bin, M.anchor, ver, conic, t.max) {
  ...
  if (is.null(M.anchor)) {
    if (length(b) == 1 && all(A == matrix(1, ncol = n, nrow = 1))) { 
      M.anchor <- infmat(Fx, b*od_REX(Fx, crit = "A", t.max = t.max/5, 
                                      track = FALSE)$w.best, echo = FALSE)
    } else {
      w.app <- od_MISOCP(Fx, b1, A1, b2, A2, b3, A3, bin = bin, t.max = t.max/3,
                         type = "approximate", crit = "A", gap = NULL, echo = FALSE)$w.best
      if (is.null(w.app)) {
  ...
  M1 <- solve(M.anchor); M2 <- M1 %*% M1
  
  if (conic) {
    
    s <- m*(m + 1)/2; Gm <- matrixcalc::duplication.matrix(m); eps <- 1e-12
    vec.M2 <- matrixcalc::vec(M2)
    tilde.h <- t(Gm) %*% vec.M2
    
    if (ver == "+") tilde.Q <- t(Gm) %*% (-tcrossprod(vec.M2) / sum(diag(M1)) +
                                           0.5 * M1 %x% M2 + 0.5 * M2 %x% M1) %*% Gm
    if (ver == "-") tilde.Q <- (1/6) * t(Gm) %*% (M1 %x% M2 + M2 %x% M1) %*% Gm
    ...
    if (ver == "+") Q <- -h %*% t(h)/(sum(diag(M1))) + V1*V2
    if (ver == "-") Q <- (1/3)*V2*V1
